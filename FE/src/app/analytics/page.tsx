...
export default function AnalyticsDashboard() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      await Promise.all([
        fetchStats(user.id),
        fetchLabels(user.id),
        fetchDetections(user.id),
        fetchDailyStats(user.id, days),
      ]);
    } finally {
      setLoading(false);
    }
  ...
  const fetchDetections = async (uid: number) => {
    try {
      const response = await axios.get(`${API_BASE}/api/yolo/user/${uid}?limit=50`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const fetchDailyStats = async (uid: number, daysValue: number) => {
    try {
      const response = await axios.get(
        `${API_BASE}/api/yolo/daily/${uid}?days=${daysValue}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleDaysChange = async (newDays: number) => {
    setDays(newDays);
    if (user?.id) {
      await fetchDailyStats(user.id, newDays);
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-orange-500">
            ...
            <select
              ...
              className="w-full px-3 py-2 border rounded text-orange-600 font-bold"
            >
                  ...
                  >
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="text-sm font-bold text-indigo-600">
                        {label.count}x ({label.percentage}%)
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-32 bg-gray-200 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${Math.min(
                              (stat.count / Math.max(...dailyStats.map((s: any) => s.count), 1)) * 100,
                              100
                            )}%`,
                          }}
                        ></div>
